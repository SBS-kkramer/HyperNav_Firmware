...
#define DLY_US(n)	cpu_delay_us(n, FOSC0)	/* Delay n microseconds */

#define	CS_H()		gpio_set_gpio_pin(EMMC_RST)	/* Set MMC CS "high" */
#define CS_L()		//gpio_clr_gpio_pin(EMMC_RST)	/* Set MMC CS "low" */
#define CK_H()		gpio_set_gpio_pin(EMMC_CLK)	/* Set MMC SCLK "high" */
#define	CK_L()		gpio_clr_gpio_pin(EMMC_CLK)	/* Set MMC SCLK "low" */
#define DI_H()		gpio_set_gpio_pin(EMMC_CMD)	/* Set MMC DI "high" */
#define DI_L()		gpio_clr_gpio_pin(EMMC_CMD)	/* Set MMC DI "low" */
#define DO			gpio_get_pin_value(EMMC_DAT)	/* Get MMC DO value (high:true, low:false) */

#define	INS			(TRUE)	/* Card is inserted (yes:true, no:false, default:true) */
#define	WP			(FALSE)	/* Card is write protected (yes:true, no:false, default:false) */


static void init_port (void)
{
	/* CS=H, SCK=L, DI=H, DO=pu, INS=pu, WP=pu */
	CS_H();
	CK_L();
	DI_H();
}

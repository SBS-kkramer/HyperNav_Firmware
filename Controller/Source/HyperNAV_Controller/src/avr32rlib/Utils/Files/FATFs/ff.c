...
#else
	res = FATFs_f_sync(fp);		/* Flush cached data */
#if _FS_SHARE
...
)
{
...
#if _USE_STRFUNC >= 2
	if (c == '\n') FATFs_f_putc ('\r', fil);	/* LF -> CRLF conversion */
#endif
	...
	btw = 1;
#endif
	FATFs_f_write(fil, s, btw, &bw);		/* Write the char to the file */
...
)
{
	...
	for (n = 0; *str; str++, n++) {
		if (FATFs_f_putc(*str, fil) == EOF) return EOF;
	}
	return n;
}
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		case 'S' :					/* String */
			...
			chc = 0;
			if (!(f & 2)) {
				while (j++ < w) chc += (cc = FATFs_f_putc(' ', fil));
			}
			chc += (cc = FATFs_f_puts(p, fil));
			while (j++ < w) chc += (cc = FATFs_f_putc(' ', fil));
			if (cc != EOF) cc = chc;
			continue;
		case 'C' :					/* Character */
			cc = FATFs_f_putc((TCHAR)va_arg(arp, int), fil); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		default:					/* Unknown type (passthrough) */
			cc = FATFs_f_putc(c, fil); continue;
		}
		...
		j = i; d = (f & 1) ? '0' : ' ';
		res = 0;
		while (!(f & 2) && j++ < w) res += (cc = FATFs_f_putc(d, fil));
		do res += (cc = FATFs_f_putc(s[--i], fil)); while(i);
		while (j++ < w) res += (cc = FATFs_f_putc(' ', fil));
		if (cc != EOF) cc = res;
	}

/*!	\file	lis3dsh.c
	\brief	Driver for LIS3DSH accelerometer
	
	Based on STMicroelectronics LIS3DH generic driver.
	Initially written for use with I2C (TWI) interface
	
	History:
		2016-01-20, Scott Feener:	started.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
uint8_t LIS3DSH_ReadReg(uint8_t twiaddr, uint8_t Reg, uint8_t* Data) 
{
	...
	packet.chip = twiaddr;		// the accelerometer address
	packet.addr[0] = Reg;					
	packet.addr_length = sizeof(uint8_t);
	packet.buffer = received_data;
	packet.length = sizeof(received_data);
	
	 // Perform a multi-byte read access then check the result.
	 while (twi_master_read(LIS3DSH_TWI_PORT, &packet) != TWI_SUCCESS)
	 {
		 io_out_string("...trying\r\n");
	 }
	
	*Data = received_data[0];
	
	//portDBG_TRACE("rd pkt: %#lX %#X %u %lu data:%#X", packet.chip, packet.addr[0], packet.addr_length, packet.length, received_data[0]);
	io_out_S16("Data Read failed (err %hd) \r\n ", (S16)check);
	return 1;
}
...
uint8_t LIS3DSH_WriteReg(uint8_t twiaddr, uint8_t Reg, uint8_t Data) 
{
	...
	packet.addr[0] = Reg;
	packet.addr_length = sizeof(uint8_t);
	packet.buffer = (void *)send_data;
	packet.length = sizeof(Data);
	
	// Perform a multi-byte write access then check the result.
	while (twi_master_write(LIS3DSH_TWI_PORT, &packet) != TWI_SUCCESS);
	
	//portDBG_TRACE("wr pkt: %#lX %#X %u %lu data:%#X", packet.chip, packet.addr[0], packet.addr_length, packet.length, Data);
...
{
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_WHO_AM_I, val) )
		return MEMS_ERROR;
...
LIS3DSH_status_t LIS3DSH_SetODR(uint8_t twiaddr, LIS3DSH_ODR_t ov)
{
	...
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_CTRL_REG4, &value) )
		return MEMS_ERROR;
	
  io_out_string ( "RdReg\r\n" );
  vTaskDelay((portTickType)TASK_DELAY_MS(1000));

	value &= 0x0F;		// Clear ODR bits
	value |= ov<<4;		// Set ODR bits
	
	if( !LIS3DSH_WriteReg(twiaddr, LIS3DSH_CTRL_REG4, value) )
		return MEMS_ERROR;
...
LIS3DSH_status_t LIS3DSH_SoftReset(uint8_t twiaddr)
{
	uint8_t value;
#if 0	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_CTRL_REG3, &value) )
		return MEMS_ERROR;
		
	value |= (1<<LIS3DSH_STRT);
	
	if( !LIS3DSH_WriteReg(twiaddr, LIS3DSH_CTRL_REG3, value) )
		return MEMS_ERROR;
#else
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_CTRL_REG6, &value) )
		return MEMS_ERROR;
	
	value |= (1<<LIS3DSH_BOOT);
	
	if( !LIS3DSH_WriteReg(twiaddr, LIS3DSH_CTRL_REG6, value) )
		return MEMS_ERROR;	
#endif

	#warning use freertos equivalent function?
	#warning check for mcu clk change
...
LIS3DSH_status_t LIS3DSH_FIFOFull(uint8_t twiaddr, uint8_t* Data)
{
	...
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_FIFO_SRC, &value) )
		return MEMS_ERROR;
	
	portDBG_TRACE("FIFO_SRC: %#X  (ck:%#X)", value, (1<<LIS3DSH_OVRN_FIFO));
		
	if (value & (1<<LIS3DSH_OVRN_FIFO))	
		*Data = 1;
	else 
		*Data = 0;
...
LIS3DSH_status_t LIS3DSH_DataAvailable(uint8_t twiaddr, uint8_t* Data)
{
	...
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_STATUS_REG, &value) )
		return MEMS_ERROR;
	
	//portDBG_TRACE("STATUS: %#X  (ck:%#X)", value, (1<<LIS3DSH_ZYXDA));
	
	if (value & (1<<LIS3DSH_ZYXDA))
		*Data = 1;
	else
		*Data = 0;
...
/*******************************************************************************
* Function Name  : LIS3DSH_GetTemp
* Description    : Read the Temperature Value
* Input          : Buffer to empty
* Output         : Temperature Values Registers buffer  (1 LSB/deg)
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
LIS3DSH_status_t LIS3DSH_GetTemp(uint8_t twiaddr, int8_t* buff) 
{
	...
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_T, &value) )
		return MEMS_ERROR;
	
	portDBG_TRACE("t = %u", value);
...
{
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_STATUS_REG, val) )
		return MEMS_ERROR;
...
LIS3DSH_status_t LIS3DSH_GetAccAxesRaw(uint8_t twiaddr, AxesRaw_t* buff) 
{
	...
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_X_L, valueL) )
	return MEMS_ERROR;
	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_X_H, valueH) )
	return MEMS_ERROR;
	
	buff->AXIS_X = value;
	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_Y_L, valueL) )
	return MEMS_ERROR;
	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_Y_H, valueH) )
	return MEMS_ERROR;
	
	buff->AXIS_Y = value;
	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_Z_L, valueL) )
	return MEMS_ERROR;
	
	if( !LIS3DSH_ReadReg(twiaddr, LIS3DSH_OUT_Z_H, valueH) )
	return MEMS_ERROR;

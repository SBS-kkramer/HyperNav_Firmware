...
static void EOS_SPEC_A_irq (void)
{
  ...
  {
    if  (received_EOS_A < 16)
      which_EOS_A[received_EOS_A] = 'A';
  ...
  {
    if  (received_EOS_A < 16)
      which_EOS_A[received_EOS_A] = 'u';
  }

  received_EOS_A ++;
	
	if  (request_clear_A > 0  ||  request_acqui_A == 0)
  {
    ...
    else
    {
      if ( cgs_autoClearing )
        ...
        previous_action_A = CGS_Undefined;
    }
	}
...
static void EOS_SPEC_B_irq(void)
{
  ...
  {
    if  (received_EOS_B < 16)
      which_EOS_B[received_EOS_B] = 'A';
  ...
  {
    if  (received_EOS_B < 16)
      which_EOS_B[received_EOS_B] = 'u';
	}

  received_EOS_B ++;
	
	if  ( request_clear_B > 0  ||  request_acqui_B == 0)
  {
    //  Start a clearing readout when no data acquisition is done,
	  //  or when clearouts requested (those are normally requested before acquisitions)
	  //
    ...
    else
    {
      if  (cgs_autoClearing)
        ...
        previous_action_B = CGS_Undefined;
    }
	}
...
S16  CGS_StartSampling (spec_num_t spec,
                        F64        inttime_ms,
                        int        num_clearouts,
                        int        num_readouts, 
                        U32*       spec_timeOfCompletion_ast
                       )
{
  ...
  if  (spec_timeOfCompletion_ast) 
    *spec_timeOfCompletion_ast = 0;

  //  Make sure integration time is in acceptable interval
  //
  if  ( !(inttime_min_ms < inttime_ms && inttime_ms < inttime_max_ms))
    return CGS_FAIL;
  ...
  else if  (num_clearouts > 15)
    num_clearouts = 15;

  if      (num_readouts <  0)
    num_readouts =  0;
  
  else if  (num_readouts > 15-num_clearouts)
    num_readouts = 15-num_clearouts;
...
void  CGS_Power_On (bool A, bool B)
{
  if  (!cgs_Is_16V_On ())
  {
    // turn on 16V supply if necessary
    cgs_16V_On ();
  }

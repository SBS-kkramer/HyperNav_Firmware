...
static char* packet_filename( const char* data_dir, const char* profile, const char* packet, const char* type ) {

  if ( !profile || 5 != strlen(profile) ) return 0;
  if ( !packet  || 4 != strlen(packet ) ) return 0;
  if ( !type    || 4  < strlen(type   ) ) return 0;

  char* fn = malloc ( data_dir?strlen(data_dir):2 + 32 );

  if ( fn ) {
    if ( !data_dir || !data_dir[0] ) {
      sprintf ( fn, "%s/%s/%s%s.pkt", ".",      profile, type, packet );
    } else {
      sprintf ( fn, "%s/%s/%s%s.pkt", data_dir, profile, type, packet );
    }
  }
...
static int packet_mkdir( char* fn ) {
...
int info_packet_save ( Profile_Info_Packet_t* pip, const char* data_dir, const char* type ) {
  ...
  char profile_yyddd[6]; snprintf ( profile_yyddd, 6, "%05hu", pip->PROF_num );
  char packet_number[5]; snprintf ( packet_number, 5, "%05hu", pip->PCKT_num );

  char* fn = packet_filename ( data_dir, profile_yyddd, packet_number, type );
  if ( !fn ) {
    fprintf ( stderr, "info_packet_save() - have no file name\n" );
    return 1;
  }

  packet_mkdir( fn );

  int fd = open ( fn, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR|S_IWUSR );

  if ( fd < 0 ) {
    fprintf ( stderr, "info_packet_save() - cannot open '%s' for writing\n", fn );
    free ( fn );
    return 1;
  } else {
    free ( fn );
...
int data_packet_save ( Profile_Data_Packet_t* packet, const char* data_dir, const char* type ) {

  if ( !packet ) return 1;

  char profile_yyddd[6]; snprintf ( profile_yyddd, 6, "%05hu", packet->PROF_num );
  char packet_number[5]; snprintf ( packet_number, 5, "%04hu", packet->PCKT_num );

  char* fn = packet_filename ( data_dir, profile_yyddd, packet_number, type );
  if ( !fn ) {
    fprintf ( stderr, "data_packet_save() - have no file name\n" );
    return 1;
  }
  ...
  if ( fd<0 ) {
    free ( fn );
    fprintf ( stderr, "data_packet_save() - cannot open '%s' for writing\n", fn );
    return 1;
  } else {
    free ( fn );
...
int data_packet_retrieve ( Profile_Data_Packet_t* p, const char* data_dir, const char* type, uint16_t profile_id, uint16_t number_of_packet ) {
  ...
  char profile_yyddd[6]; snprintf ( profile_yyddd, 6, "%05hu", profile_id );
  char packet_number[5]; snprintf ( packet_number, 5, "%04hu", number_of_packet%10000 );

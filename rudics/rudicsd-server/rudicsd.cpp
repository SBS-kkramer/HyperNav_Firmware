...
#define VERSION "$Revision: 0.4 $ $Date: 2009/03/10 17:09:55 $"
...
# else
static const char *logincmd = "/usr/bin/su";
# endif
static const char *progname = "rudicsd";
static char tty[1024];
static time_t To=0L;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * $Id: rudicsd.cpp,v 0.4 2009/03/10 17:09:55 swift Exp $
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * Copyright (C) 2006 Dana Swift
 *
 ...
 *
 * $Log: rudicsd.cpp,v $
 * Revision 0.4  2009/03/10 17:09:55  swift
 * Minor changes to conform to changing standards expressed in Fedora 10.
 *
...
int main(int argc, char **argv)
{
   ...
   for (opterr=0; (opt=getopt(argc,argv,"?hit:"))!=EOF;)
   {
      switch (opt)
      {
         ...
         default: {RudicsLog=false; syserr("usage: %s -? -h -i -t[Min]\n",progname);}
      }
   }
...
static void cleanup(void)
{
   ...
   *p='p'; chmod(tty,0666); chown(tty,0,0);
   ...
   // make a syslog entry
   sysmsg("Rudics connection terminated[%s]: UnixEpoch: %lds "
...
# if USE_OLD_LOGIN
   ...
   sleep(3); execl(logincmd,logincmd,NULL);

# else
   //  Satisfy caller expectations:
   write ( "login:", 6, 1, 0 );
   sleep ( 3 );

   execl(logincmd,logincmd,username,NULL);
# endif
...
static void sigalrm(int signo)
{
   ...
   syserr("$WARNING$ - terminating (orphan?) iridium connection.\n");
}
